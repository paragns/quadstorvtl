...
Requires: httpd, tar, coreutils, sg3_utils, libcom_err
...
install -m 644 /quadstorvtl/quadstor/etc/quadstorvtl.service $RPM_BUILD_ROOT/usr/lib/systemd/system/quadstorvtl.service
...
%post
	...
	if [ $1 -eq 1 ]; then
		systemctl enable quadstorvtl > /dev/null 2>&1
	fi

	/usr/sbin/setsebool -P httpd_enable_cgi 1 > /dev/null 2>&1

	mkdir -p /quadstorvtl/etc
	echo "2.2.16 for Rocky Linux X" > /quadstorvtl/etc/quadstor-vtl-core-version
...
%preun
	if [ $1 -eq 0 ]; then
		systemctl stop quadstorvtl > /dev/null 2>&1
		systemctl disable quadstorvtl > /dev/null 2>&1
	fi
