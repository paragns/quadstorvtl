...
#define DEBUG_INFO_NEW(fmt, args...)                                           \
  printk(KERN_INFO "%s:%d " fmt, __FUNCTION__, __LINE__, ##args)
#else
...
#define DEBUG_WARN_NEW(fmt, args...)                                           \
  printk(KERN_WARNING "WARN:%s:%d " fmt, __FUNCTION__, __LINE__, ##args)
#define DEBUG_CRIT_NEW(fmt, args...)                                           \
  printk(KERN_CRIT "CRIT:%s:%d" fmt, __FUNCTION__, __LINE__, ##args)
#ifdef ENABLE_STATS
...
static inline void wait_chan_init(wait_chan_t *chan, const char *name) {
...
#define wait_on_chan_interruptible(chan, condition)                            \
  wait_event_interruptible(chan, condition)
#define wait_on_chan_timeout(chan, condition, timeout)                         \
  wait_event_timeout(chan, condition, timeout)
...
/*
 * FIX: free/malloc/zalloc are defined as macros here in the original.
 * This corrupts struct field names in exportdefs.h/commondefs.h when
 * those headers are included after this one (e.g. a field named 'free'
 * becomes 'kfree' at the preprocessor level).
 *
 * Solution: define them with the QS_ prefix for use in .c files,
 * and also define the short names ONLY when explicitly requested
 * via QS_DEFINE_SHORT_ALLOC_MACROS, which core_linux.c does NOT set
 * (it calls kfree/kmalloc/kzalloc directly).
 *
 * All other quadstor .c files that use free()/malloc()/zalloc() and
 * are NOT kernel module files can define QS_DEFINE_SHORT_ALLOC_MACROS
 * before including this header if needed.
 */
#define qs_free(ptr, type) kfree(ptr)
#define qs_malloc(s, type, flags) kmalloc(s, flags)
#define qs_zalloc(s, type, flags) kzalloc(s, flags)

/* M_DEVBUF/M_QUADSTOR are BSD-ism type tags, unused by Linux allocators */
#define M_DEVBUF 0
#define M_QUADSTOR 0
...
/*
 * FIX: mtx_lock/mtx_unlock are NOT defined as macros here anymore.
 * The original #define mtx_lock spin_lock corrupted .mtx_lock field
 * names in struct qs_kern_cbs.  core_linux.c calls spin_lock/spin_unlock
 * directly.  Other files that need the short names can define them locally.
 */
...
/*
 * FIX: printf/sprintf/snprintf NOT defined as macros here.
 * Original #define printf printk corrupted .printf field in qs_kern_cbs.
 */
#define slab_cache_create kmem_cache_create
#define slab_cache_destroy kmem_cache_destroy
static inline void *slab_cache_alloc(struct kmem_cache *cachep,
                                     allocflags_t flags, size_t len) {
  void *ret;
...
/*
 * FIX: copyin/copyout NOT defined as macros here.
 * Original definitions corrupted .copyin/.copyout fields in qs_kern_cbs.
 * core_linux.c uses copy_from_user/copy_to_user directly.
 */
...
/*
 * FIX: pause NOT defined as macro here — corrupted .pause field in qs_kern_cbs.
 */
...
/*
 * FIX: ticks_to_msecs NOT defined as macro here — corrupted field name.
 * Use jiffies_to_msecs() directly in code that needs it.
 */
...
static inline unsigned long pgdata_page_address(struct pgdata *pgdata) {
