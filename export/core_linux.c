...
static int sys_sock_has_write_space(sock_t *sys_sock) {
  return (sk_stream_wspace(sys_sock->sock->sk) >=
          sk_stream_min_wspace(sys_sock->sock->sk));
}

static int sys_sock_read(sock_t *sys_sock, void *buf, int len) {
  struct msghdr msg;
  struct kvec iov;
  int retval;
...
static void sys_sock_free(sock_t *sys_sock) {
  sock_release(sys_sock->sock);
  kfree(sys_sock);
}

static void sock_deactivate(sock_t *sys_sock) {
...
static int sys_sock_write_page(sock_t *sys_sock, pagestruct_t *page, int offset,
                               int len) {
  struct socket *sock = sys_sock->sock;
...
static int sys_sock_write(sock_t *sys_sock, void *buf, int len) {
  struct msghdr msg;
  struct kvec iov;
  int retval;
...
static sock_t *sys_sock_create(void *priv) {
  sock_t *sys_sock;
  int retval;
...
static void sock_activate(sock_t *sys_sock) {
  struct socket *sock = sys_sock->sock;
...
static void sys_sock_nopush(sock_t *sys_sock, int set) {
  kernel_setsockopt(sys_sock->sock, SOL_TCP, TCP_CORK, (void *)&set,
                    sizeof(set));
}

static sock_t *sys_sock_accept(sock_t *sys_sock, void *priv, int *error,
                               uint32_t *ipaddr) {
  sock_t *new_syssock;
  struct socket *sock = sys_sock->sock;
  ...
  new_syssock->saddr_len = retval;
#else
  if (newsock->ops->getname(newsock, (struct sockaddr *)&new_syssock->saddr,
                            &new_syssock->saddr_len, 2)) {
    sys_sock_close(new_syssock, 1);
    sys_sock_free(new_syssock);
    return NULL;
  }
  ...
  kernel_setsockopt(newsock, SOL_TCP, TCP_NODELAY, (void *)&disabled,
                    sizeof(disabled));
...
static int sys_sock_bind(sock_t *sys_sock, uint32_t addr, uint16_t port) {
  struct socket *sock = sys_sock->sock;
  ...
  kernel_setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (void *)&reuse,
                    sizeof(reuse));
  retval =
...
static int sys_sock_connect(sock_t *sys_sock, uint32_t addr,
                            uint32_t local_addr, uint16_t port) {
  struct socket *sock = sys_sock->sock;
  struct sockaddr_in saddr_in;
  int disabled = 1;
  int retval;
  ...
  saddr_in.sin_addr.s_addr = addr;
  sock_activate(sys_sock);
  retval = sock->ops->connect(sock, (struct sockaddr *)&saddr_in,
...
static void debug_info(char *fmt, ...) {}
...
static void uma_zfree(uma_t *cachep, void *ptr) {
  kmem_cache_free(cachep, ptr);
}

static void *__zalloc(size_t size, int type, allocflags_t aflags) {
  int flags = (aflags & Q_NOWAIT_INTR) ? GFP_ATOMIC : GFP_NOIO;
  int wait = (aflags & Q_WAITOK) ? 1 : 0;
  void *ret;
...
#undef __malloc
#endif
static void *__malloc(size_t size, int type, allocflags_t aflags) {
...
#undef __free
#endif
static void __free(void *ptr) { kfree(ptr); }
...
static void cv_free(cv_t *cv) { kmem_cache_free(cv_cache, cv); }
...
static long cv_timedwait(cv_t *cv, mtx_t *mtx, void *data, int timo) {
  unsigned long *flags = data;
  DEFINE_WAIT(wait);
  long ret;
...
static void bdev_marker(iodev_t *b_dev, struct tpriv *tpriv) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 0))
...
static void bdev_start(iodev_t *b_dev, struct tpriv *tpriv) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 0))
  if (tpriv->data) {
...
static void __pause(const char *msg, int timo) { msleep(timo); }
...
static int __kernel_thread_check(int *flags, int bit) {
...
int bio_unmap(iodev_t *iodev, void *cp, uint64_t start_sector, uint32_t blocks,
              uint32_t shift, void (*end_bio_func)(bio_t *, int), void *priv) {
  int diff = (shift - 9);
  if (diff) {
    start_sector <<= diff;
    blocks <<= diff;
  ...
  return blkdev_issue_discard(iodev, start_sector, blocks, GFP_NOIO,
                              DISCARD_FL_WAIT)
             ? -1
             : 1;
#else
  return blkdev_issue_discard(iodev, start_sector, blocks, GFP_NOIO, 0) ? -1
                                                                        : 1;
#endif
}
#else
...
bio_t *g_new_bio(iodev_t *iodev, void (*end_bio_func)(bio_t *, int),
  ...
  bio = bio_alloc(GFP_NOIO, bio_vec_count);
  if (unlikely(!bio))
    return NULL;
...
static void bio_free_page(bio_t *bio) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0)
...
static uint32_t bio_get_max_pages(iodev_t *iodev) {
...
static int __copyin(void *uaddr, void *kaddr, size_t len) {
